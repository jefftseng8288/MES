...
## 2026-08-01 — 第五種靜默失效:跑的不是最新 code

- **★★ 「code 改了」≠「跑的是新 code」—— 常駐程序把程式碼凍結在啟動當下(2026-08-01)。**
  `com.mes.harvest` 是 `KeepAlive=true` 的**常駐** daemon,啟動於 2026-07-15 22:04,之後**跑了 16 天,從未載入過任何一次 code 變更**。期間我們改了批量、挑選邏輯、新增 5 個 Seed 來源、加了心跳、加了評論類 feature —— **對它全都不存在**。
  **最惡毒的是它完全沒有訊號:** log 照常寫、批次每 3 小時準時跑、觀測照常寫入 DB,**看起來一切正常**。它是靠「產出的形狀不對」才被抓到的 —— 8/1 06:00 那批是 **3 店 × 9 feature = 27 筆、零評論 feature**,而新 code 應是 15 店 × 12 feature ≈ 180 筆。
  **為什麼只有它中招:** launchd 的**一次性** job(`RunAtLoad=false` + 無 `KeepAlive`:projection / insight / alarm)每次觸發都由 launchd **重新 spawn 一個新程序**,必然載入磁碟上的最新 code;**常駐 job 不會**。這也解釋了為何 projection / insight 前一天才掛上去、當晚 23:30 / 23:40 就精準運作,而 harvest 掛了 16 天卻在跑舊邏輯。
  **這是「掛上去 ≠ 會執行 ≠ 有產出」的第四層:** 即使「有掛、會執行、有產出」,**跑的仍可能是舊邏輯**。前三層問「它有沒有在動」,這一層問「**動的是哪一版**」。
  **修法(治標):** 改動常駐 daemon 涉及的 code 後**必須重啟並驗證版本**。
  **修法(治本,已實作):** 心跳自動帶 `code_version`(git HEAD 短 hash),每日安好會比對並標示「⚠️ 跑的是舊 code(xxx,目前 yyy)—— 常駐 daemon 需重啟」。**把「跑的是哪版」從需要推理的事,變成看得見的事。**

- **★ 版本 hash 必須在 import 時求值,不能延後到執行時(2026-08-01,一個會讓整個機制失效的陷阱)。**
  心跳要回報「我跑的是哪版 code」。若在**執行時**才呼叫 `git rev-parse HEAD`,讀到的是**磁碟現況** —— 一個跑著舊 code 的常駐 daemon 會回報「磁碟上的新 hash」,**反而把問題掩蓋掉**(顯示一切正常)。必須在 **import 時**求值並凍結成模組常數,才會誠實反映「我啟動時載入的那一版」。
  **通則:任何「描述執行主體自身狀態」的值,取值時機必須與該狀態的凍結時機一致。** 問錯時間點,得到的是別人的答案。

- **★ 錯的診斷比不報更糟 —— 它把人推向錯的方向(2026-08-01)。**
  7/31 的警鈴報「02:00 observed=0 → 批次無記錄 → **執行異常(daemon 沒跑)**」。但真相是 baseline **有跑**,只是一個新 Seed 都沒撈到(健康報告白紙黑字:批次筆數 0 / 目標 30)。根因:診斷只讀 `observation_log`,而**「沒跑」與「跑了但撈到 0」在 observation_log 上長得一模一樣**(都是查無此批號)。
  **值得注意的是:心跳當時已經做好了,只是既有診斷還沒改讀它** —— **加了新的資訊源,不等於既有的判斷邏輯會自動變聰明**。新增觀測能力時,要一併回頭檢查「哪些既有推論,現在有更好的依據了」。
  修法:診斷先讀心跳 —— 有心跳 → 排除執行異常,往供給/限流方向判斷;無心跳 → 才報執行異常。

---

## 2026-08-02 — 框架預設值不等於合理值(今天第二次同一個模式)

- **★★ 引入框架時,要主動確認關鍵預設值 —— 別假設「預設就是合理」(2026-08-02)。**
  今天一天之內,同一個模式咬了兩次,兩次都是**框架的預設值在靜默地做錯事**:
  1. **`APScheduler.misfire_grace_time` 預設 = 1 秒。** 事件迴圈只要卡超過 1 秒,該次任務就被**整個丟棄**(不是延後,是跳過),只在 stderr 留一行字。實測 8/1 因為約 **2 秒**的停頓,`12:00 harvest`、`21:00 baseline`、`21:00 harvest` **三批直接消失**。更糟的是:err log 顯示 **7/24 就有 `missed by 1.22s`** —— **這個系統一直在用「卡頓超過 1 秒就丟掉整批」的設定在跑,只是沒人看 err log 所以不知道。**
  2. **`APScheduler.max_instances` 是 per-job,不是全域**(前一天踩到)。以為設了就不會重疊,實際只擋「同一個 job 疊自己」,三個 baseline slot 是三個 job,互相之間完全不設防。
  **共同形狀:「以為框架幫你處理好了,其實沒有」,而且失效時是靜默的** —— 沒有例外、沒有警告,只有「事情就是沒發生」。
  **據此立下的做法:引入或設定框架時,對「會影響正確性的關鍵參數」主動查清楚預設值是什麼、合不合理**,不要等它出事。特別留意這三類:①**容錯窗**(grace / timeout / retry)—— 預設常常極短;②**併發控制**(max_instances / pool size / lock 範圍)—— 作用域常常比想像中窄;③**失敗處理**(丟棄 vs 重試 vs 拋錯)—— 預設常是「安靜丟棄」。
  **並且:框架把事情丟掉時,要讓它留下結構化訊號。** 本次做法是接 `EVENT_JOB_MISSED` 事件寫進 `job_run_log`(`status='missed'`),而不是去解析 stderr 文字 —— 事件是正式 API,文字格式會變;而且結構化之後「被丟棄」才變成可查詢、可聚合的資料。

- **★ 「被丟棄」不能算成「有跑」—— 否則修好一個訊號會弄瞎另一個(2026-08-02)。**
  把 misfire 記進 `job_run_log` 之後有個陷阱:若讓它跟 success/failed 一樣被當成心跳,那麼「該跑而沒跑」的警鈴會被它餵飽而**啞掉** —— 明明整批沒執行,警鈴卻看到「有心跳」。所以 `load_job_beats` 只認 `RAN_STATUSES = (success, failed)`,`missed` 另外計數。
  **通則:新增一種記錄狀態時,要回頭檢查「所有既有的『有沒有記錄』判斷」是否需要排除它。** 加一種狀態不是只加一種狀態,而是動到了每一個讀這張表的推論。

- **★ 版本標記在工作樹髒時必須標 `-dirty`(2026-08-02,當天就實際發生)。**
  `code_version` 用 `git rev-parse --short HEAD` 時,daemon 明明跑著「含未 commit 改動的 code」,卻回報一個**乾淨的 commit hash** —— 又是「看起來對、其實不對」。而且它**正好在最需要它的場景失真**:開發中、剛改完還沒 commit,恰恰是最容易搞混「跑的是哪版」的時候。就在今天重啟 daemon 時真實發生了一次。改用 `git describe --always --dirty --abbrev=7` → `b7ea865-dirty`。
  **通則:任何「自我標示身分/版本」的機制,都要問一句「它在**不完美的狀態**下會不會說謊」** —— 完美狀態下人人都誠實,機制的價值全在邊緣情況。

---

## 2026-08-03 — Phase 3:AI 第一次進場,以及它照出來的東西

- **★★ 靠 import 副作用建立的 registry,會「測試全過、生產全掛」(2026-08-03)。**
  第一次真實 API 呼叫,9 條 LLM 產出被擋下 **6 條**,錯誤是 `insight_type 'SKU_SCALE' 未登記(已登記:[])` —— **registry 是空的**。根因:`SKU_SCALE` 由 `insight_producers` 在**類別定義時**透過 `__init_subclass__` 登記,而入口點 `python -m mes.hypothesis` **從未 import 到那個模組**。
  **而測試全過** —— 因為測試檔自己 import 了 producers。**測試環境與生產環境的 import 圖不同**,這個差異在單元測試裡完全看不見。
  **通則:任何靠 import 副作用建立的註冊機制(registry / plugin / hook),都有這個風險。** 本專案目前有兩個(insight producers、predicate)。處置:**在需要 registry 的模組裡放顯式 import**,把隱性依賴變成顯性(並註明理由,否則會被當成無用 import 而被「清理」掉)。
  **這是既有那條家族的新一層:** 「掛上去 ≠ 會執行 ≠ 有產出 ≠ 跑的是新 code」,現在再加一層 —— **測得過 ≠ 生產跑得起來**。四層的共同形狀都是「**只有真的跑一次入口點才會暴露**」。

- **模型 id 憑記憶寫 → 404(2026-08-03,又一次)。**
  寫了 `claude-opus-4-20250514`,實測不存在於此帳戶;改成**查 API**(`client.models.list()`)取得實際可用的 id。與「App Store handle 不能憑 app 名猜」是同一條:**碰外部世界的東西,先探再寫。** 值得注意的是這次連「模型名稱」這種看似記得住的東西也踩到 —— **「我記得」不是查證。**

- **★★ 受控清單「先留空、讓真實需求填」的價值,在第一次生成就兌現了(2026-08-03)。**
  predicate registry 刻意只登記 `SWAP_APP_INTENT` 一個,不預先窮舉(理由:合法值取決於還沒定的 Phase 4 武器)。第一次真實生成,LLM 一再想用未登記的 predicate:`NO_APP_ADOPTION_INTENT`(不裝)、`ADOPT_APP_INTENT`(新裝)、`LOCALIZATION_APP_INTENT`。
  **這正是機制的價值:讓真實的生成需求告訴我們該登記什麼,比憑空窮舉好。** 它揭露了一件我們原本沒想到的事 —— **`SWAP`(換掉現有的)涵蓋不了「新裝」與「不裝」,那是三種不同的市場行為**。若當初憑空窮舉,很可能只列出一堆 email 相關的 predicate,反而漏掉這個真正的維度。
  Jeff 據此登記 `ADOPT` / `NO_ADOPTION`,構成**互斥且窮盡的行為三態**。

- **★ 兩個正交維度不該壓成一個欄位(2026-08-03,`LOCALIZATION_APP_INTENT` 被拒的理由)。**
  LLM 也想要 `LOCALIZATION_APP_INTENT`,Jeff 拒絕。判準:**「行為意圖」(換/裝/不裝)與「產品範疇」(想裝哪一類 app)是兩個正交維度** —— 混進同一個 predicate 欄位,值域會隨 app 類別數量**相乘爆炸**(review×3、shipping×3、localization×3…)。
  **通則:當一個受控欄位的候選值開始出現「A 的 X」「B 的 X」這種形狀時,那是兩個維度被壓成一個的訊號 —— 該拆成兩個欄位,不是繼續往清單裡加值。**
  第一版的處理:這類資訊記在自由文字的 `rationale` 裡,**不另開欄位** —— 等真實需求證明需要再結構化(同「沒有下游行為就不設門檻」的節制)。

- **★ AI 不得代替人做審核決策 —— 這次實際發生並被擋下(2026-08-03)。**
  提示詞要求「實際用它審核那 6 條假說」,但 Jeff 只對其中 1 條給了明確裁決。**其餘 5 條留白未動。** 三個理由:① approve/reject 是**商業判斷**,不是執行細節;② Roadmap 的 Phase 3 停止條件明載「**AI 做 approve 以外的決策 → 停**」;③ `decision.actor='jeff'` —— 代按等於**把話塞進他嘴裡**,而那些理由是**未來演化的素材**,偽造的判斷會污染整條演化鏈。
  **這條界線的特別之處:它不是「做錯事」的防線,而是「做對事但沒有資格做」的防線。** 那 5 條我或許能給出看起來合理的判斷,但合理不等於有資格 —— **決策權的歸屬本身就是系統設計的一部分。**

- **LLM 提出「pattern 該再切一刀」→ 拒絕擴充語法,保留為 Phase 5 的真實 test case(2026-08-03)。**
  LLM 不只套用給定的 pattern,還提出「決定變數應是**價格**而非商品數,sub-$35 子群接受率更高」—— 這是它自己發現了 pattern 的再切分。但現行 pattern 只支援 **insight 等值條件(AND)**,表達不了 `avg_price < 35` 這種**數值範圍**;若照原 pattern 執行,Phase 4 撈到的店會涵蓋所有價位,**與假說想打的對象不一致**。
  Jeff 裁決:**reject,但不為單一 LLM 奇想擴充 pattern 語法**(不破壞目前穩定的等值聚合器)。`rejection_reason` 誠實記下「pattern 精細度超出目前聚合能力」。
  **判準:一個好的建議 + 一個尚未被需求證明的擴充 = 仍然不做。** 但**把它記成 reject 理由,而不是丟掉** —— 它成為 Phase 5 演化 pattern 語法時的**真實 test case**,比屆時憑空想像需求好得多。

---

## 2026-08-04 — 歸因困境:未受控的變數會讓實驗白做

- **★★ 實驗要能歸因,前提是「其他變數已固定」—— 否則失敗了也學不到東西(2026-08-04,Phase 4 暫緩的理由)。**
  Phase 4 的價值**不在「接觸到人」,而在「假說能被證偽」** —— 而證偽要求**結果只反映假說的對錯**。
  Jeff 決定暫緩 Phase 4 等 EscapeFlow v2 完成,理由是:若產品沒準備好就開始接觸,實驗失敗會有**兩個無法分離的解釋** —— **(a) 假說錯了**(市場不在乎這個提案,是該學的市場事實)、**(b) 產品還不行**(商家其實有興趣、看了卻沒被說服,與假說對錯無關)。
  **關鍵在於:兩者的「結果」長得一模一樣(沒反應 / 沒安裝),含意卻完全相反** —— 一個該讓我們修正對市場的認知,另一個該讓我們去修產品。
  **而且危害不只是「白做」:會把「產品沒準備好」誤記成「市場不要」,那筆錯誤的 Outcome 會一路污染 Phase 5 的假說演化。** 錯誤的證偽比沒有證偽更貴 —— 沒有證偽只是原地踏步,錯誤的證偽會讓系統往錯的方向演化,而且**看起來像在學習**。
  **這其實是 Phase 4 自身「單變數原則」的直接推論**,只是變數不在實驗設計裡、而在環境中:**產品狀態若未固定,就等於一次測了兩件事。** 通則:**盤點變數時不能只盤點「我打算改的」,還要盤點「同時也在動的」** —— 後者不會出現在實驗設計文件上,卻一樣會毀掉歸因。
  **判準:與其收一批無法歸因的資料,不如先把變數固定下來再開始。** 資料不是越多越好 —— **無法歸因的資料不只是沒用,它會被當成有用。**

---

## 2026-08-12 — 地雷引爆:阻塞式 sleep 吃掉整批,而且沒有任何訊號

- **★★ 在 asyncio 裡用阻塞式 `time.sleep()`,會讓排程整批消失 —— 而且 misfire 偵測抓不到(2026-08-12,已實際發生)。**
  這顆地雷 2026-08-02 就標記過(「baseline 一批阻塞 52 分鐘,若跑滿 5 小時煞車會吃掉期間所有 harvest」),**8/11 真的引爆**:baseline 02:00 那批跑了 **93 分鐘**,期間 APScheduler 賴以運作的事件迴圈完全停擺,**03:00 的 harvest 整批消失**。
  **最惡毒的是它沒有留下任何失敗訊號:** `status='missed'` 的記錄總數是 **0** —— 上週才加的 `EVENT_JOB_MISSED` 監聽器**涵蓋不到這種情況**,因為**迴圈停擺時 APScheduler 自己也停擺**,等它恢復,`coalesce` 把那次合併掉了,不算 misfire。**唯一的訊號是每日安好裡的 `harvest 7/8`** —— 一個沒人盯就會滑過去的數字。
  **而且趨勢在惡化:** Seed 供給修好後每批真的撈滿 30 筆,baseline 每批耗時從 8/05 的 64 分長到 8/11 的 **87 分**,**每天阻塞約 4.4 小時**,而 harvest 每 3 小時一次 —— **碰撞從「可能」變成「必然」,只是何時發生。**
  **修法(選 to_thread 而非改 5 處 sleep):** 把整批工作丟進 worker thread(`asyncio.to_thread` + 內層 `asyncio.run`),讓排程用的迴圈保持自由。**節流邏輯本身沒問題,問題只在「它跑在哪個執行緒」** —— 改成 `await asyncio.sleep()` 要把整條同步鏈路(scrape / inference / harvest / reviews 的 httpx)全改 async,牽動極廣,而 to_thread 讓批次內部**一行都不用動**。
  **通則:框架的「非同步」是一種契約 —— 你借用它的迴圈,就有義務不佔住它。** 一個同步函式放進 async 介面裡「看起來會動」,但它偷走的是**整個系統的排程能力**,而症狀會出現在**完全不相干的地方**(這次是另一條鏈路的批次消失)。

- **★ 分不出「有影響」與「沒差」的警告,會把自己訓練成雜訊(2026-08-12)。**
  `code_version` 的 stale 偵測直接比 git HEAD,結果 **8/11 的每日安好把「跑的是舊 code —— 需重啟」亮了出來,但那次 `5a435c9 → e8b0341` 只動了 4 個文件檔**,daemon 跑的邏輯與 HEAD 完全相同,重啟不會改變任何行為。
  本專案文件 commit 很頻繁 → 這個警告會**幾乎每天亮**,然後被學會忽略,**真正該重啟的那次就會被漏掉**。
  修法:版本標記仍用 git hash(可讀、可查),但**判斷是否警告時只看 `src/` 與 `prompts/` 有沒有差異**(`git diff --name-only <running>..HEAD -- src prompts`)。無法比較時(任一邊 `-dirty`、commit 不存在、git 失敗)**保守回報 stale** —— 寧可多叫一次,不可漏掉。
  **這與「產出為 0 要能分辨正常閒置與異常」是同一條原則的第二次應用:警告的價值不在「有沒有偵測到差異」,而在「這個差異值不值得你採取行動」。** 判準:**設計警告時要問「它在『沒事』的情況下會不會亮」** —— 會亮就得再收斂,否則它遲早會被當成背景雜訊。

---

## 2026-08-13 — 同一個病的第二個入口(以及地雷修復的生產驗證)

- **★ 修好一個誤報,要順著它的「入口」全部走一遍 —— 否則同一個病會從旁邊那扇門回來(2026-08-13,隔一天就復發)。**
  8/12 才把 stale 偵測收斂成「只看 `src/` 與 `prompts/` 的差異」,**8/13 的每日安好又亮了同一個誤報** —— 這次的路徑不同:`running=c44a324`、`current=c44a324-dirty`,而 `-dirty` **完全來自三個未 commit 的文件檔**,`src/` 與 `prompts/` 是乾淨的,daemon 跑的 code 與磁碟**一模一樣**。
  **原因是保守分支的位置:** `code_is_stale` 一看到任一邊帶 `-dirty` 就 `return True`,**在做 path-scoped 比對之前就投降了** —— 上次修的是「純文件 **commit** 讓 HEAD 前進」,這次是「純文件的**未 commit 改動**讓工作樹變髒」。**同一個病,兩個入口,而我只堵了一個。**
  而且第二個入口更頻繁:commit 是離散事件,「正在寫文件、還沒 commit」卻是**連續的一段時間** —— 警告會整段時間一直亮。
  修法:`-dirty` 也套用同一條判準 —— 先問 `git status --porcelain -- src prompts`「**髒的是不是 code 路徑**」,不是一看到 `-dirty` 就投降。保守只保留在**真正無從判斷**的地方:**running 端**帶 `-dirty`(那棵工作樹當時長怎樣已無從回溯)、髒在 code 路徑、或 git 查不出來。
  **通則:「保守預設」不是免罪金牌 —— 放錯位置的保守會製造誤報,而誤報的終點是被忽略,那等於沒有警告。** 判準:每次為某個判斷加保守分支時,問一次「**它會在多少『其實沒事』的情況下觸發**」;答案若是「很常」,那個保守就放錯位置了 —— 該做的是**把不確定的範圍縮小**,而不是擴大投降的條件。

- **★ 生產驗證:阻塞式 sleep 的修法有效(2026-08-13,同一 slot 的前後對照)。**
  8/11、8/12 連兩天,`02:00` baseline(93 / 84 分鐘)吃掉 `03:00` 的 harvest;修正 8/12 09:26 上線後:
  **8/12 21:00 baseline(84 分)與 21:00 harvest 同時觸發,兩批都完成**;
  **8/13 02:00 baseline(87 分)覆蓋期間,03:00 harvest 正常跑完(7 分鐘)** —— **正是連兩天消失的那一格**。
  且 harvest 耗時與平時(5–8 分)無異 —— **兩條鏈路是真的並行,不是排隊。**
  **驗證的形狀值得記:最強的證據是「同一個位置、同樣的條件、修正前後不同結果」** —— 不是「跑起來沒報錯」,也不是「PID 換了」。而這個對照能成立,是因為**失效當時留下了可比對的記錄**(`job_run_log` 的每批起訖時間);沒有那些記錄,修完也只能說「應該好了」。
