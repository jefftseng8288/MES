...
### ★ 真問題:阻塞式 `time.sleep()` 吃掉整批 harvest

```
baseline 02:00 → 03:33(93 分鐘,阻塞事件迴圈)
harvest  03:00 → 整批消失,且無 missed 記錄
```

這顆地雷 8/02 就標記過,8/11 真的引爆。**misfire 偵測抓不到** —— 迴圈停擺時 APScheduler
自己也停擺,恢復後 `coalesce` 把它合併掉。**唯一訊號是每日安好的 `7/8`。**

趨勢惡化中:Seed 供給修好後每批撈滿 30 筆,baseline 每批耗時 64 分(8/05)→ **87 分(8/11)**,
**每天阻塞約 4.4 小時**,harvest 每 3 小時一次 → 碰撞已是必然。

**修法:** 整批工作丟進 worker thread(`asyncio.to_thread` + 內層 `asyncio.run`),
事件迴圈保持自由。**選 to_thread 而非把 5 處 sleep 改 async** —— 節流邏輯本身沒問題,
問題只在「它跑在哪個執行緒」;改 async 要動整條同步鏈路,to_thread 讓批次內部一行都不用改。
副作用(可接受):baseline 與 harvest 現在可真正並行 —— 那本來就是設計意圖(兩條鏈路戳不同對象),
先前的序列化只是阻塞造成的副作用。

**測試用「事件迴圈探針」驗證**:批次阻塞期間跑一個 async ticker,若迴圈被卡住 tick 數會遠少於預期。

### 假警報:「跑的是舊 code」在純文件 commit 後也會亮

`5a435c9 → e8b0341` 只動了 4 個文件檔,daemon 跑的邏輯與 HEAD 完全相同,警告卻說「需重啟」。
本專案文件 commit 頻繁 → 這個警告會幾乎天天亮,然後**被學會忽略,真正該重啟的那次就漏掉**。

**修法:** 版本標記仍用 git hash,但判斷是否警告時**只看 `src/` 與 `prompts/` 有無差異**。
無法比較時(任一邊 `-dirty`、commit 不存在)**保守回報 stale**。
與「產出為 0 要分辨正常閒置與異常」同一條原則的第二次應用。

測試 281 passed(+6);ruff / mypy 綠。

---

## 2026-08-04 — Phase 4 暫緩(等 EscapeFlow v2)+ daemon 重啟對齊 code_version

### Phase 4 → ⏸️ 暫緩(Jeff 定案)

**狀態改為「暫緩(等 EscapeFlow v2 完成)」,並把理由寫進 task_plan 與 Roadmap** ——
否則幾週後回來只會看到「未開始」,不知道當初為什麼停。

**理由是歸因困境:** Phase 4 的價值不在「接觸到人」,而在「**假說能被證偽**」——
那要求**結果只反映假說的對錯**。若 v2 未完成就接觸,失敗會有兩個無法分離的解釋:
**(a) 假說錯了**(市場事實)、**(b) 產品還不行**(與假說無關)。
**兩者結果長得一模一樣(沒反應/沒安裝),含意完全相反** —— 分不出來等於什麼都沒學到,
更糟的是會把「產品沒準備好」誤記成「市場不要」,**污染 Phase 5 的假說演化**。

這是 Phase 4 自身「單變數原則」的直接推論(產品狀態是未受控的變數)。
同時把「產品狀態未固定就接觸 → 停」加進停止條件,並在 Roadmap 的進入條件補上
「EscapeFlow v2 完成」。**等待不是閒置** —— 期間四條鏈路照常累積 Observation/Knowledge/Insight。

### daemon 重啟,對齊 code_version

工作樹確認乾淨(HEAD `5a435c9`,`code_version` 解析無 `-dirty`)後重啟
`com.mes.harvest`(PID 76313 → 75352,16:16)。
**PID 換了不算證據** —— 決定性證據是**常駐 daemon 自己產出的下一筆心跳帶 `5a435c9`**
(刻意只認 `harvest`/`baseline`;projection/insight 是一次性 job,每次重新 spawn 本來就會
載入最新 code,拿它們當證據是假證據)。已掛背景等待,下次觸發 18:00。

> 對照:前天重啟時工作樹是髒的,心跳標成 `a18abee-dirty` —— 那筆版本資訊其實不可信。
> 這次先確認乾淨再重啟,`5a435c9` 才是可信的標記。**`-dirty` 標記的價值在此兌現。**
