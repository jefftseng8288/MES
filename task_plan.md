...
**狀態:** 🔄 **能力大部分達成,換模型待第二個 provider**(三批全完成:第一批 schema / 第二批-A 生成 / 第二批-B 審核;循環已實跑通:生成 → 審核 → decision 進 Decision Graph)

> **★ 刻意不標 ✅。** 四條驗收裡有兩條**無法驗證**(需要第二個 LLM provider),而 Jeff 明確表示短期不辦第二組 key、不排時程。若直接標 ✅,未來看到的人會**誤以為 P4(Model Agnostic)已被驗證** —— 那是把未驗證當成已驗證。詳見下方驗收逐條對照。
...
**狀態:** 🔄 **2/4 達成,2/4 無法驗證(已知缺口,非待辦)**

**驗收條件逐條對照(2026-08-03 實跑後):**

| 驗收條件 | 狀態 | 依據 |
|---|---|---|
| 假說結構化、帶 evidence、引用 Insight、可審核 | ✅ **達成** | 真實產出 6 條,皆帶 `pattern` / `source_insight_refs` / `rationale` / 版本三欄;審核頁可讀 |
| reject 進 Decision Graph | ✅ **達成** | 已實際 reject 一條,decision 寫入且決策史可還原 |
| 換模型(GPT ↔ Claude)讀同一份 Insight 各自產生假說 | ⚠️ **無法驗證** | 只有一個 provider |
| 可分別評估模型的觀察力與推理力 | ⚠️ **無法驗證** | 需兩個模型才能比較 |

> ### ★ 換模型那兩條:**已知的驗收缺口,不是待辦事項**
>
> Jeff 明確表示**短期不辦第二組 API key,不排時程** —— 所以這裡記錄為「缺口」,
> 不列進工作清單、不設期限。
>
> **必須守住的區分:「架構就緒」≠「驗收通過」。**
> 抽象層設計**已證明正確** —— 整條生成流程用 `MockProvider` 跑得通、**一次 API 都沒打**;
> 補 `OpenAIProvider` 只需新增一個類別 + `_PROVIDERS` 加一行,**呼叫端完全不動**。
> **但那不等於 P4(Model Agnostic)已經被驗證** —— P4 的主張是「換掉模型系統照樣運作」,
> 而那件事**從未被實際執行過一次**。設計看起來對,和實際換過一次,是兩種不同強度的證據。
>
> **為什麼要標得這麼細:** 否則未來看到 Phase 3 ✅,會誤以為 P4 已驗證過。
> **把未驗證當成已驗證,是最貴的一種錯**(它會讓人在錯誤的地基上繼續往上蓋)。

**其他誠實標記:**
- **假說品質未經評估** —— Phase 3 驗的是**形狀**不是**準度**,證偽在 Phase 4。
  那 6 條讀起來合理,但「讀起來合理」不是證據。
- **審核只做了 1 條** —— 另外 5 條維持 pending,**等 Jeff 自己判斷**
  (approve/reject 是商業判斷,AI 不代勞,見 CLAUDE.md)。
...
**狀態:** ⏸️ **暫緩(等 EscapeFlow v2 完成,預計 2026 年 8 月底)** — 2026-08-04 Jeff 定案

> ### ★ 回來動工前必讀:[`docs/MES_Positioning_Revision_v1.md`](docs/MES_Positioning_Revision_v1.md)
>
> 2026-08-12 的一輪概念討論,挖出**定位層級的認知修正**(產品也是變數,不是常數)。
> **那份文件不是待辦清單**,但其中兩件事會**直接影響 Phase 4 的 schema 設計**,
> 且屬於「現在不做、以後補不回來」:
> 1. **Experiment 要記「完整配置」(管道 × 訊息 × 時機 × 素材 × 受眾切法),不是只記「用了哪個武器」**
>    —— 只記管道的話,未來樣本夠了也做不了統計學習,因為缺的那幾維**過去沒記就是沒記**。
> 2. **產品層的歸因需要自己這邊的漏斗數字** —— 否則失敗永遠只能歸因到假說(見下方歸因困境的一般化版本)。

**目的:** 真實接觸市場並同步收反饋,親手驗證「系統會學習」。

**進入條件:** Phase 3 通過 + Phase 3.5 至少一種合規、可收反饋的接觸行為就緒 + **EscapeFlow v2 完成**(2026-08-04 新增,理由見下)。

> ### ⏸️ 為什麼暫緩(★ 理由要留著,否則幾週後回來只會看到「未開始」,不知道當初為什麼停)
>
> **四個理由,核心是最後一個(歸因困境)。**
>
> 1. **v2 所有「完成」的功能都還沒上 production** ——
>    商家現在看到的,與我們要行銷的,**不是同一個東西**。
> 2. **武器庫第一順位「App Store listing 優化」的前提是功能已出貨** ——
>    功能沒上線,listing 就寫不出來(要寫什麼?)。第一順位卡住,後面的順位更沒意義。
> 3. **有正在傷害真實用戶的 bug** —— Loox 影片評論靜默遺失,**免費版現在就在發生**。
>    在這種狀態下導流,等於把更多人帶去踩同一個坑。
> 4. **★ 歸因困境(核心):產品沒準備好,實驗失敗了也無法歸因。**
>
> Phase 4 的價值**不在「接觸到人」,而在「假說能被證偽」** —— 一條假說預測某類商家會有反應,
> 我們去打,然後**從結果學到東西**。但這件事成立的前提是:**結果只反映假說的對錯。**
>
> 若 EscapeFlow v2 還沒完成就開始接觸,實驗失敗時會有**至少兩個無法分離的解釋**:
> 1. **假說錯了** —— 這類商家根本不在乎這個提案(市場事實,該記進 Outcome 演化假說);
> 2. **產品還不行** —— 商家其實有興趣、點進來看了,但產品當下的狀態說服不了他
>    (**與假說對錯無關**,是我們自己的問題)。
>
> **兩者的「結果」長得一模一樣(沒反應 / 沒安裝),但含意完全相反** ——
> 一個該讓我們修正對市場的認知,一個該讓我們去修產品。**分不出來就等於什麼都沒學到,
> 而且更糟:會把「產品沒準備好」誤記成「市場不要」,污染整條演化鏈**
> (那筆錯誤的 Outcome 會一路影響 Phase 5 的假說演化)。
>
> **這正是 Phase 4 自己「單變數原則」的直接推論** —— 一次只測一個變數。
> 產品狀態若是個未受控的變數,那就等於同時測了兩件事。
> **與其收一批無法歸因的資料,不如先把產品變數固定下來。**
>
> **★ 暫停期間沒有真正的等待成本:** 時間上與 `GROWTH_VELOCITY` 需要的**約一個月觀測期重疊** ——
> 就算現在不暫緩,那一維也還是得等。而暫緩期間 MES 的四條鏈路照常運轉,
> Observation / Knowledge / Insight 持續累積 —— **等待不是閒置**,
> 資料變厚會讓 Phase 4 起跑時的假說更有層次
> (目前 Insight 只有 `SKU_SCALE` 一維,pattern 只有 3 種)。
>
> **解除條件:** EscapeFlow v2 完成(預計 2026 年 8 月底)。
> 屆時重新評估 Phase 3.5 的接觸前置條件是否也就緒。
...
**狀態:** ⬜ 未驗收(Phase ⏸️ 暫緩中,等 EscapeFlow v2)
...
**★ 新增(2026-08-04):產品狀態未固定就開始接觸 → 停**(那等於一次動了兩個變數,結果無法歸因)。
