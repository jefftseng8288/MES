...
- **⚠️ GROWTH_VELOCITY 的資料源歷程(誠實性標記,2026-07-31 更新):**
  - **原本(Phase 2.5 完成時):無資料源。** 它算的 `review_count` 不在 Feature Taxonomy v1 的 9 個市場特徵內、Phase 1-D 沒抓,真實觀測數為 0 → **對真實資料永遠產出 0 筆**。當時特別標明:這**不是「還不夠 30 天、等等就有」,而是源頭沒開** —— 兩者都表現為「暫時沒資料」,但一個會自己好、一個永遠不會。
  - **現在:資料源已開通。** `review_count` / `avg_rating` / `rating_distribution` 已納入 Taxonomy v2 並實作 loox handler,真實觀測已有(6 家,7~1590 則)。
  - **仍為 0 的原因已改變:** 成長率需要同一家店**相隔約 30 天的兩個觀測點**,而 harvest 最小重抓間隔為 7 天 → 最快**約一個月後**才有第一筆。**這次是真的「還沒累積夠」,會自己好。**
