...
## 五原則 + 兩條補充條款(系統原則 / 工程基線 — 可演化,非教條)

> **定性(口徑):** 以下六大原則(P1~P6)是「當前時空背景下,為確保系統不腐敗而立下的高效率圖紙」,是**系統原則 / 工程基線**,**不是不可修改的教條**,而是可因真實世界回饋(Evidence)動態優化、版本升級的演進工具。
> 它們之上還有兩層:**唯一絕對底線**是法律紅線(絕不用 MES 做任何違法或惡意詐欺;見下),**最高依據**是終極目的(創造實質行銷效益)。系統原則是被這兩層檢驗的,不是拿來否決它們的。三層口徑詳見 `CLAUDE.md` A+ 節。
> 註:此為口徑框架的釐清(2026-07-12),六原則的內容與資料層硬約束皆不變。
...
| 5 | Principles(已完成) | — | 系統原則 / 工程基線(可演化,非教條),其餘四份都要能追溯到它 |
...
> 驗收驗的是「能力是否具備」,不是「連續跑了多久」。時間・區間(連續 N 天、規模到 N 家)不列為驗收條件——持續運行是系統存在的常態,不是要達成的目標。

- 能把 Seed 轉成帶完整 Provenance、五 metadata + entity 歸屬齊全、結構正確的 Observation。
- 失敗誠實不偽裝(observed / fetch_failed / not_found 三值,寫入層強制);Append-Only 沒覆蓋。
- 撞真實世界(限流、種子池乾)能誠實反應,且可透過加來源持續運行。
- → 不是「能抓」就成功,是「抓進來的資料乾淨且結構對、失敗誠實」才成功。
...
> **實況更新(2026-07-31,不改上述定位,補一則已被市場修正的認知):**
> Discovery 雖仍 Deferred,但 Seed 來源這件事已被實況推著往前走了兩步,兩步都值得記進前提:
>
> 1. **Seed 來源已不限 review app。** 供給不足時擴充了 5 個「有規模才會裝」的工具類型
>    (email 行銷 / 忠誠度 / 訂閱制 / 跨境多語言),並實測確認 **App Store 的 selector 跨 app 類型通用**
>    —— 所以「加一個來源」的成本是加一個 handle,不是寫一套新解析。這使
>    「從單一 review app 長成更廣的來源」不再是預期,而是**已經發生的事實**。
>    但要留意:**「有規模才會裝」目前仍是啟發(Hypothesis 性質),不是已驗證的事實** ——
>    來源品質要等這批店被 harvest 完(is_active / product_count / review_count)才有資料可判。
> 2. **★「來源枯竭」的判斷兩次都是錯的。** 兩次「池子乾了」的診斷,真實原因都是
>    `MAX_PAGES` 的視野限制(實測各來源第 25 頁仍滿),不是市場沒有店。
>    **這對 Discovery 的前提有直接影響:** 未來評估「某個 Discovery 管道還有沒有料」時,
>    必須先確認**是不是我們自己的取樣限制**,否則會把「我們沒看」誤判成「那裡沒有」,
>    進而錯誤地放棄一個其實還很肥的管道。已修:觸頂改為主動回報且與「真的翻到底」型別上分開。
...
+ **EscapeFlow v2 完成**(2026-08-04 新增)。

> **⏸️ 暫緩中(2026-08-04 Jeff 定案):等 EscapeFlow v2 完成(預計 2026 年 8 月底)。**
>
> **★ 回來動工前必讀:[`MES_Positioning_Revision_v1.md`](MES_Positioning_Revision_v1.md)**
> —— 2026-08-12 的概念討論(**非待辦清單**),其中兩件事會直接影響本 Phase 的 schema:
> **Experiment 要記「完整配置」而非只記管道**、**產品層歸因需要自己這邊的漏斗數字**。
> 兩件都屬於「現在不做、以後補不回來」。
>
> **其他暫緩理由(v2 功能未上 production / listing 寫不出來 / 有正在傷害用戶的 bug)
> 見 [`../task_plan.md`](../task_plan.md) Phase 4 段落。**
>
> **理由是歸因困境:產品沒準備好,實驗失敗了也無法歸因。** Phase 4 的價值不在「接觸到人」,
> 而在「**假說能被證偽**」—— 而那要求**結果只反映假說的對錯**。若 v2 未完成就開始接觸,
> 失敗會有兩個無法分離的解釋:**(a) 假說錯了**(市場事實,該演化假說)、
> **(b) 產品還不行**(與假說無關,是我們自己的問題)。
> **兩者的結果長得一模一樣(沒反應/沒安裝),含意卻完全相反** —— 分不出來等於什麼都沒學到,
> 更糟的是會把「產品沒準備好」誤記成「市場不要」,**那筆錯誤的 Outcome 會一路污染 Phase 5 的假說演化**。
> 這是 Phase 4 自身「**單變數原則**」的直接推論:產品狀態若是未受控的變數,就等於同時測了兩件事。
>
> **等待不是閒置** —— 期間四條鏈路照常運轉,Observation / Knowledge / Insight 持續累積,
> Phase 4 起跑時的假說會比現在更有層次。
...
- **單變數原則**:一次只測一個變數(A/B 核心,隔離變數才能歸因)。
  **★ 2026-08-12 重新定位(見 [`MES_Positioning_Revision_v1.md`](MES_Positioning_Revision_v1.md) 第四節):**
  單變數不是「每次接觸只能有一個變數」,而是「**要比較 A 和 B 時,其他條件要固定**」。
  **配置(這次接觸的完整參數)與對照(兩個配置只差一維)是兩件事** ——
  **必做的是記錄完整配置;單變數只在要做對照時適用。**
  它是**樣本稀缺期的工具**,不是永久原則(組合拳 = 統計上的交互作用,單變數天生看不見)。
...
> **★ 2026-08-12 概念討論的修正(尚未落成設計,見 [`MES_Positioning_Revision_v1.md`](MES_Positioning_Revision_v1.md) 第三節):**
> 下面這份清單只涵蓋**「管道」這一維** —— 而一次接觸的完整配置是
> **管道 × 訊息 × 時機 × 素材 × 受眾切法**。
> 其中「受眾切法」= Pattern(已有)、「訊息/素材」= 影片工作流那條線、
> **「時機」目前完全空白**(但 Append-Only 的時間序列理論上已具備偵測能力)。
> **這不改變下面的優先序,但影響 Experiment 該記什麼。**
