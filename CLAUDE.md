...
- **★「掛上去了」≠「會執行」≠「執行有產出」—— 三件事要分別驗證(2026-07-31 血淚)。** 部署類工作(daemon / 排程 / plist)不可只確認「檔案寫好了」或「已 load」就宣稱完成。三層各有各的驗法:①**有沒有掛** → `launchctl list`;②**會不會執行** → **實際觸發一次**(如 `launchctl kickstart`),看程序真的起來、log 真的被寫入;③**執行有沒有產出** → 看 DB 真的多了列。實例:`projection` / `insight` 的 plist 寫好放在 repo 裡卻**從未被 load**(卡在第一層);`store_harvest` 有 load、有跑、log 每 3 小時都在寫,卻**連續 16 天原地打轉零產出**(卡在第三層)。**只驗前一層就說「已部署」,是同一個形狀的錯。**
- **★ 新增一條鏈路時,「把它接進監控」是同一批工作的一部分。** 橫切監控若只覆蓋部分鏈路,等於沒有 —— 而且比沒有更危險,因為它給了「有在看」的錯覺。實例:警鈴只監測 baseline,另外三條鏈路靜默失效兩週,期間警鈴每天照常回報正常。
- **★ 改動常駐 daemon 涉及的 code 後,必須重啟並驗證版本(2026-08-01 血淚)。** launchd 的**一次性** job(projection / insight / alarm)每次觸發都重新 spawn,**必然載入最新 code**;但**常駐** job(`KeepAlive=true`,目前是 `com.mes.harvest`)會把程式碼**凍結在程序啟動當下** —— 實測它跑了 **16 天舊 code**,期間所有改動對它都不存在,而且 log 照常、批次照常、**完全沒有訊號**。這是「掛上去 ≠ 會執行 ≠ 有產出」的**第四層:動的是哪一版**。重啟後要驗證(心跳的 `code_version` 應等於當前 git HEAD)。
- **★ 引入框架時,主動確認關鍵預設值 —— 別假設「預設就是合理」(2026-08-02 血淚,一天內咬兩次)。** 實例:`APScheduler.misfire_grace_time` 預設**僅 1 秒**,事件迴圈卡超過 1 秒該次任務就被**整個丟棄**(實測 2 秒的停頓吃掉三批,且從 7/24 就在發生沒人知道);`max_instances` 是 **per-job** 不是全域(只擋同一 job 疊自己)。**共同形狀:「以為框架幫你處理好了,其實沒有」,而且失效時是靜默的。** 特別查這三類預設:①**容錯窗**(grace / timeout / retry,常常極短)②**併發控制**(作用域常比想像窄)③**失敗處理**(預設常是安靜丟棄)。**且框架丟掉東西時,要讓它留下結構化訊號**(接事件寫進 DB,不要只留 stderr 一行字、更不要去解析文字)。
- **★★ 不得代替 Jeff 做「判斷類」決策 —— 即使被要求(2026-08-03 實際發生)。** 假說的 approve / reject、要不要登記某個受控值、某條策略要不要採用 —— 這些是**商業判斷**,不是執行細節。Roadmap 的 Phase 3 停止條件明載「**AI 做 approve 以外的決策 → 停**」;且 `decision.actor='jeff'`,代按等於**把話塞進他嘴裡**,而那些理由是**未來演化的素材**,偽造的判斷會污染整條演化鏈。**做法:只執行 Jeff 明確裁決過的,其餘留白並回報。** 實例:曾被要求「用審核頁審完那 6 條假說」,但 Jeff 只裁決了 1 條 → 只執行那 1 條,其餘 5 條維持 pending。**這條防的不是「做錯事」,是「做對事但沒有資格做」** —— 決策權的歸屬本身就是系統設計的一部分。
- **★ 靠 import 副作用的 registry,要在使用端放顯式 import(2026-08-03 血淚)。** 用 `__init_subclass__` / 裝飾器 建立的註冊機制(本專案:insight producers、predicate),**測試會過但生產會掛** —— 因為測試檔自己 import 了那個模組,而生產入口點沒有。實例:第一次真實生成,9 條產出被擋下 6 條,錯誤是「insight_type 未登記(已登記:[])」。**做法:在需要 registry 的模組放顯式 import 並註明理由**(否則會被當成無用 import 清掉)。這是「掛上去 ≠ 會執行 ≠ 有產出 ≠ 跑的是新 code」家族的第四層:**測得過 ≠ 生產跑得起來**。
- **★ 分不清「我們的限制」與「世界的事實」,就會反覆誤判。** 任何「因為我們設的上限/能力邊界而停止」的情況,都必須與「因為外界真的沒有了而停止」**分開回報**。實例:`MAX_PAGES=12` 翻不滿時只回報「撈到 2 家」,與「市場真的沒店了」長得一模一樣 → 同一個誤讀發生兩次。這是「失敗不偽裝」在**邊界**上的版本。
